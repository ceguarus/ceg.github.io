...
ims.interfaz = {
	...
	resize : function() {
		...
		if(typeof(editable)=="undefined") {
			...
			$('.pop_up').css("left", ($('#limit').width() - $('.pop_up').outerWidth(true,true)) / 2).css("top", ($('#limit').height() - $('.pop_up').outerHeight(true,true)) / 2);
	...
	showStars : function(obj) {
		var stars = new Array();
		for(var n = 0; n<=5;n++){
			var id = "st_"+obj.attr("id")+"_"+n;
			var myX = parseInt(obj.offset().left-$("#visor").offset().left),
				myY = parseInt(obj.offset().top-$("#visor").offset().top);
		...
		var mTop=0,
			...
			index = 0,
			second = false;
		$.each(stars,function(i,id){
			var _this = $("#"+id);

			index++;
			if(index<=3&&second==false){
				var increment = index+"0";
				mTop = parseInt(_this.css("top"))-parseInt(increment);
				_this.css("left",parseInt(_this.css("left"))+obj.width());
				mLeft = parseInt(_this.css("left"))+parseInt(increment);
			}else{
